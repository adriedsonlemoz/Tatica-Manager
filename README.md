...
- integra 15 falas geradas pelo usuário em uma mesma voz masculina para início, cartões, falta, pênalti, defesa, trave, substituição, lesão, gols, intervalo, segundo tempo e fim de jogo;
- separa a chamada de gol do mandante e do visitante usando o lado real do evento já informado pela partida, sem alterar o Match Engine;
- normaliza os clipes em WAV mono 24 kHz, com recorte de silêncio e fades curtos para evitar cliques, mantendo volume consistente sobre o ambiente;
- reproduz as falas por `just_audio` em um player dedicado, permitindo voz e efeitos simultâneos sem substituir a torcida ou os sons de campo;
- mantém o TTS do Android apenas como fallback caso um asset de locução não possa ser carregado;
- amplia o ducking da torcida nos eventos mais longos para a nova locução continuar inteligível até o fim.

## Áudio de partida renovado — 0.1.1.145

- substitui o ambiente contínuo anterior por uma faixa de estádio gerada pelo usuário, tratada em WAV 48 kHz e preparada com costura de loop para reduzir cortes e artefatos de compressão;
- integra novos efeitos tratados de chute, bola na trave, defesa do goleiro, gol e pênalti defendido sem alterar o Match Engine nem a ordem dos eventos;
- mantém os demais efeitos existentes quando os clipes novos não ficaram suficientemente isolados, evitando trocar sons por trechos ambíguos;
- reduz o ambiente de 18% para 16% do volume efetivo da partida, mantendo torcida ao fundo e liberando espaço para efeitos e narração;
- preserva personalização de sons, opção de áudio limpo, TTS, músicas do menu e controles existentes.
...
- corrige os quatro erros `undefined_getter` do workflow 90;
- importa diretamente as extensões que fornecem os rótulos de formação, mentalidade, pressão e ritmo à tela do Auxiliar técnico;
- preserva integralmente a IA de treinamento, as recomendações e as regras do Match Engine da versão anterior.
...
- mantém placar, campo, timeline, comandos, estatísticas e narração na mesma tela, sem rolagem vertical, com adaptação proporcional para aparelhos menores;
- usa a formação real, acompanha a fase da jogada, separa jogadores próximos e mantém micro movimentos contextuais sem mover a bola parada artificialmente;
- reduz a poluição de nomes: ao vivo aparecem jogador ativo, envolvidos e atletas próximos da bola; na pausa, todos podem ser identificados;
- melhora hierarquia do placar, evento sobre o campo, contraste dos uniformes nas estatísticas e cartões separados por equipe;
- mostra cartões, risco de suspensão e suspensão ativa na Escalação, no Elenco e no perfil do jogador, sempre pela competição da próxima partida;
- uniformiza os modais da partida no fundo azul-grafite e preserva integralmente probabilidades, resultados e regras do Match Engine.
...
- atualiza a asserção estrutural antiga que ainda exigia o resolvedor agregado de partidas em segundo plano;
- protege o comportamento atual: todas as partidas CPU usam o único `MatchEngine`, geram eventos completos e permitem substituições automáticas;
- não altera regras, resultados ou interface e preserva o `flutter analyze` aprovado no log 85.
...
- refaz **Calendário** com cabeçalho do clube, abas Mês/Agenda/Resultados, calendário mensal, compromissos paginados e resumo, usando somente fixtures e eventos persistidos;
- refaz **Táticas** com campo vertical, titulares, banco, formação e os cinco controles já usados pelo Match Engine, sem inventar Instruções ou Bolas paradas;
- refaz **Configurações** em tela fixa com áudio, vibração, velocidade, duração, bola, perfil, carreira, novidades, contato e apoio;
- conecta a velocidade 1x/2x/4x ao relógio visual da partida sem modificar resultado, estatísticas ou regras do Match Engine;
- mantém as três páginas principais sem rolagem e usa redução responsiva apenas quando a altura disponível é menor.
