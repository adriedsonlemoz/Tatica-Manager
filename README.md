...
- `VERSION` — versão visível simples (`0.1.1.65`);
- `app.json` — identidade externa do aplicativo;
- `pubspec.yaml` — manifesto Flutter, com versão SemVer compatível (`0.1.1+67`);
- Android — plataforma versionada no repositório, com `versionName 0.1.1.65` e `versionCode 67`;
- iOS — catálogo `AppIcon.appiconset` com todos os tamanhos já versionado; a estrutura Xcode completa será sincronizada quando a plataforma iOS for adicionada;
- GitHub Actions — valida a versão embutida no APK antes de publicar o Artifact.

> O Flutter/Dart usa SemVer no `pubspec.yaml`, por isso a release de quatro partes `0.1.1.65` é representada internamente como `0.1.1+67`. A versão visível do aplicativo/Android continua sendo `0.1.1.65`.

## Política obrigatória de release

Toda correção, alteração, refatoração ou entrega deve atualizar a versão antes de ser publicada. O padrão visível é `A.B.C.D`; para esta linha, a próxima entrega normalmente será `0.1.1.66`, salvo quando houver um incremento funcional maior.
