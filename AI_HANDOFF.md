...
## Correção do analyzer do polimento

- O workflow 95 validou metadados e dependências, mas parou no `flutter analyze` com três causas.
- O estado temporário do Avançar Dia agora usa `NotifierProvider`, seguindo a API atual e os controladores existentes.
- `showGameNotice` recebe import explícito em `home_screen.dart`.
- O teste da apresentação da carreira teve o parêntese excedente removido.
- Não houve mudança funcional nos recursos entregues pela 0.1.1.141.

Consulte `docs/RELEASE_0.1.1.142.md`.
...
## Correção dos rótulos do auxiliar

- O workflow 90 apontou quatro `undefined_getter` apenas na apresentação dos rótulos da nova tela.
- `TechnicalAssistantScreen` agora importa diretamente `formation.dart` e `tactic.dart`, disponibilizando `FormationTypeX`, `MentalityX`, `PressingX` e `MatchTempoX` ao analyzer.
- A correção não altera recomendações, treino, escalação, tática nem Match Engine.

Consulte `docs/RELEASE_0.1.1.137.md`.
...
## Correção do analyzer do campo dinâmico

- O log 87 encontrou apenas `prefer_null_aware_operators` em `match_pitch_game.dart` e interrompeu o workflow antes dos testes e do APK.
- O renderer agora encaminha diretamente `_possessionHome`, que já é anulável, sem mudar o comportamento da movimentação por fases.
- As cinco melhorias do campo da 0.1.1.133 permanecem intactas.

Consulte `docs/RELEASE_0.1.1.134.md`.
